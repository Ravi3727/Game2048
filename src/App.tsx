...
const App = () => {
  ...
  const slideAndMergeRow = (row: number[]) => {
    ...
    return [...newRow.filter((value) => value !== 0), ...Array(4 - newRow.filter((value) => value !== 0).length).fill(0)];
  };
  ...
  const moveRight = () => {
    setGrid((prevGrid) => {
      const newGrid = prevGrid.map((row) => slideAndMergeRow([...row].reverse()).reverse());
      return newGrid;
    });
    randomGenerateBlock();
  };
  ...
  return (
    <div className="flex font-serif flex-col items-center justify-center min-h-screen bg-myCustomColor">
      <h1 className="text-4xl font-bold mb-8 text-white">Welcome to 2048 Game</h1>
      ...
      <div className="mt-8 grid grid-cols-3 gap-4">
        <button onClick={moveLeft} className="col-start-1 bg-myCustomGridColor rounded-md p-4 shadow-md text-white hover:bg-opacity-80 transition-colors">
          <ArrowLeft className="mx-auto" />
        </button>
        <button onClick={moveUp} className="col-start-2 bg-myCustomGridColor rounded-md p-4 shadow-md text-white hover:bg-opacity-80 transition-colors">
          <ArrowUp className="mx-auto" />
        </button>
        <button onClick={moveRight} className="col-start-3 bg-myCustomGridColor rounded-md p-4 shadow-md text-white hover:bg-opacity-80 transition-colors">
          <ArrowRight className="mx-auto" />
        </button>
        <button onClick={moveDown} className="col-start-2 bg-myCustomGridColor rounded-md p-4 shadow-md text-white hover:bg-opacity-80 transition-colors">
          <ArrowDown className="mx-auto" />
        </button>
      </div>

      <button onClick={resetGame} className='mt-8 bg-myCustomGridColor rounded-md p-4 shadow-md text-white hover:bg-opacity-80 transition-colors flex items-center'>
